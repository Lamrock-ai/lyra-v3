/* ─────────────────────────────────────────────────────────────────────
   L.Y.R.A — Shared runtime v2 (vanilla JS, ES2017+)
   Public API attached to window.L.Y.R.A :

     Lyra.mountAtmosphere()
     Lyra.setMode(mode)                        → "home"|"workspace"|"capacites"|"config"
     Lyra.mountRooms({ mode, pages, activePage, onNav, onMissionControl })
     Lyra.mountTopbar({ pageTitle, crumb, onAsk, onCmdK })
     Lyra.openCmdK() / .closeCmdK()
     Lyra.openMissionControl() / .closeMissionControl()
     Lyra.registerCommands(arr)
     Lyra.notify({ kind, text })
     Lyra.api.get(path) / .post(path, body)
     Lyra.fmt.num(v) / .pct(v) / .relTime(d)
     Lyra.sparkline(data, opts)
...
(function () {
  ...
  var Lyra = (window.Lyra = window.Lyra || {}); window["L.Y.R.A"] = Lyra;
  ...
  Lyra.navigate = function (url) {
    // Si on tourne dans l'iframe, déléguer au shell parent
    if (window !== window.top && window.top.Lyra?.navigateFrame) {
      window.top.Lyra.navigateFrame(url);
      return;
    }
    // Si le shell home a enregistré un handler iframe
    if (typeof Lyra.navigateFrame === "function") {
      Lyra.navigateFrame(url);
      return;
    }
  ...
  /* ───────── Mode system ───────── */
  const MODE_META = {
    home:       { chapter: "—",  num: "00", label: "L.Y.R.A",        watermark: "" },
  ...
  /* ───────── Scene card ───────── */
  Lyra.showSceneCard = function (chapter, title, duration) {
  ...
  Lyra.mountRooms = function (opts) {
    ...
    if (!mc) {
      mc = el("button", { id: "j-rooms-mc", class: "rooms-mc", onclick: () => Lyra.openMissionControl() });
  ...
  const ROOMS = [
    ...
    { mode: "capacites", label: "Capacités",      sub: "Ce que L.Y.R.A sait faire pour toi",      href: "/capabilities", chapter: "II",  pages: ["Intégrations","Skills","Routines","Ambiances","Store","Écosystème"] },
  ...
  function ensureMissionControl() {
    if (mcRoot) return;
    mcRoot = el("div", { class: "mission-overlay is-hidden", onclick: (e) => { if (e.target === mcRoot) Lyra.closeMissionControl(); } });
    ...
    header.appendChild(el("button", { class: "mc-close", onclick: () => Lyra.closeMissionControl(), text: "×" }));
    ...
    ROOMS.forEach(r => {
      const card = el("button", {
        class: "mission-card",
        dataset: { mode: r.mode },
        onclick: () => { Lyra.closeMissionControl(); Lyra.navigate(r.href); },
      });
  ...
  Lyra.openMissionControl = function () {
    ensureMissionControl();
    mcOpen = true;
    mcRoot.classList.remove("is-hidden");
  };
  Lyra.closeMissionControl = function () {
    if (!mcRoot) return;
    mcOpen = false;
  ...
  */
  Lyra.mountSidebar = function (opts) {
    ...
    root.appendChild(el("div", { class: "sb-brand" }, [
      ...
      el("div", { class: "sb-brand-text" }, [
        el("span", { class: "sb-brand-name", text: "L.Y.R.A" }),
  ...
  /* ───────── Topbar ───────── */
  Lyra.mountTopbar = function (opts) {
  ...
  const _navCmds = ROOMS.map(r => ({
    ...
    run: () => { Lyra.navigate(r.href); },
  }));
  cmdkCommands = _navCmds;

  Lyra.registerCommands = function (cmds) {
  ...
  function ensureCmdK() {
    if (cmdkRoot) return;
    cmdkRoot = el("div", { class: "cmdk-back is-hidden", onclick: (e) => { if (e.target === cmdkRoot) Lyra.closeCmdK(); } });
    ...
    cmdkInput = el("input", {
      ...
      onkeydown: (e) => {
        if (e.key === "Escape")    { e.preventDefault(); Lyra.closeCmdK(); }
  ...
  function currentResults() {
    ...
    if (v.startsWith(">")) {
      ...
      if (q.startsWith("ask ")) {
        const p = q.slice(4);
        return [{ kind: "ask", id: "ask", title: 'Demander : "' + p + '"', glyph: "›", group: "Faire", run: () => _askLyra(p) }];
      }
  ...
  async function _askLyra(prompt) {
    Lyra.notify({ kind: "info", text: "L.Y.R.A · réflexion…" });
    try {
      const r = await Lyra.api.post("/api/agent/ask", { prompt });
      Lyra.notify({ kind: "success", text: (r.answer || r.text || "").slice(0, 240) });
    } catch (err) {
      Lyra.notify({ kind: "error", text: "Échec : " + err.message });
    }
  }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if ((e.metaKey || e.ctrlKey) && (e.key === "k" || e.key === "K")) {
      e.preventDefault();
      cmdkOpen ? Lyra.closeCmdK() : Lyra.openCmdK();
    }
    // ⌘T — Mission Control
    if ((e.metaKey || e.ctrlKey) && (e.key === "t" || e.key === "T")) {
      e.preventDefault();
      mcOpen ? Lyra.closeMissionControl() : Lyra.openMissionControl();
    }
    // Escape
    if (e.key === "Escape") {
      if (cmdkOpen) Lyra.closeCmdK();
      if (mcOpen)   Lyra.closeMissionControl();
    }
  });
  ...
  /* ───────── Notification ribbon ───────── */
  let notifStack = null;
  Lyra.notify = function ({ kind = "info", text = "" }) {
    if (!notifStack) {
  ...
  /* ───────── Sparkline ───────── */
  Lyra.sparkline = function (data, opts) {
    opts = opts || {};
  ...
  Lyra.mountBottomNav = function (opts) {
