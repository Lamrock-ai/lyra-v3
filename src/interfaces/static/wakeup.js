...
function resetInactivityTimer() {
    ...
    if (window._lyraSleeping) return;
...
async function enterSleepMode() {
    ...
    gsap.to(['#L.Y.R.A-toggle', '#globe-toggle', '#intel-toggle',
...
async function showScanResult(result) {
    ...
    if (result.recognized) {
        ...
        name.textContent = (_userFirstname || result.name || 'L.Y.R.A').toUpperCase();
...
// ═══════════════════════════════════════════════════════════════════════════
//  PHASE 5 — Déploiement UI + voix L.Y.R.A
// ═══════════════════════════════════════════════════════════════════════════
...
async function deployUI() {
    ...
    tl.to(['#L.Y.R.A-toggle', '#globe-toggle', '#intel-toggle',
            '#dashboard-nav-btn', '#settings-nav-btn', '#panels-toggle'],
    ...
    // Voix L.Y.R.A (après 0.8s pour que l'UI soit visible)
    await _delay(800);
    try {
...
async function triggerWakeUp() {
    ...
    try {
        ...
        if (granted) {
            // Phase 5 — Déployer UI
            await deployUI();
            console.log('[WakeUp] Accès accordé — L.Y.R.A actif');
        } else {
