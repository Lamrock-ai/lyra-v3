...
(function () {
  ...
  async function saveSetting(key, value, btn) {
    ...
    try {
      ...
      if (resp.needs_restart) {
        J.notify({ kind: "error", text: key + " · redémarrage L.Y.R.A requis" });
      } else {
  ...
  async function renderPreferences() {
    ...
    const fnInput = el("input", { class: "input-mono", style: { width:"220px" }, value: Lyra.user_firstname || "" });
    ...
    wrap.appendChild(ghostSec("Identité", "comment L.Y.R.A s'adresse à toi", null, identList));

    // ── Langue & style ──
    const styleList = el("div");
    const qbTog = el("div", { class: "toggle" + (Lyra.quebec_mode ? " on" : "") });
    ...
    const wupTog = el("div", { class: "toggle" + (Lyra.wakeup_enabled ? " on" : "") });
    ...
    const clapTog = el("div", { class: "toggle" + (Lyra.clap_detection_enabled ? " on" : "") });
    ...
    wakeList.appendChild(settingRow("Détection de clap", "Double clap pour réveiller L.Y.R.A", clapTog));
  ...
  async function renderConso() {
    ...
    li.appendChild(sw);
    li.appendChild(document.createTextNode(" L.Y.R.A "));
    ...
    const page = pageWrapper("conso", "Ce que L.Y.R.A consomme pour fonctionner", null, wrap);
    root.innerHTML = ""; root.appendChild(page);
  }
  ...
  async function renderSysteme() {
    ...
    if (Object.keys(stats).length) {
      ...
      wrap.appendChild(ghostSec("Statistiques L.Y.R.A", "projets · mémoire · sessions", null, statGrid));
    }
    ...
    restartTxt.appendChild(el("div", { class: "danger-label", text: "Redémarrer L.Y.R.A" }));
    ...
    restartBtn.addEventListener("click", async () => {
      if (!confirm("Redémarrer L.Y.R.A ?")) return;
  ...
  async function renderApropos() {
    ...
    nameWrap.appendChild(el("div", { class: "about2-name", text: "L.Y.R.A" }));
    ...
    updateBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (res.already_up_to_date) {
          updateStatus.textContent = "L.Y.R.A est déjà à jour.";
          updateStatus.className = "about2-update-status ok";
          updateBtn.textContent = "À jour";
        } else if (res.ok) {
          updateStatus.textContent = "Mise à jour réussie. Redémarre L.Y.R.A pour appliquer les changements.";
          updateStatus.className = "about2-update-status ok";
    ...
    const page = pageWrapper("apropos", "À propos de L.Y.R.A", null, wrap);
    root.innerHTML = ""; root.appendChild(page);
  }
