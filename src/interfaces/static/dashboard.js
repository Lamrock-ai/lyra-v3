...
(function () {
  "use strict";
  const J = window.L.Y.R.A, el = J.el;
  ...
  async function loadInitiatives() {
    try {
      ...
      return raw.map(i => ({
        ...
        source:   i.context ? i.context.slice(0,40) : "L.Y.R.A",
  ...
  async function renderInitiatives() {
    ...
    wrap.appendChild(ghostSec(
      "En attente",
      inits.length + " initiatives · proactif L.Y.R.A",
      null,
      list
    ));
  ...
  function buildInitPanel(panel, i) {
    ...
    info.appendChild(el("div", { class: "mp-sub", text: i.source || "L.Y.R.A proactif · " + (i.due || "") }));
  ...
  function renderInitRow(i, showActions) {
    ...
    meta.appendChild(el("span", { text: i.source || "L.Y.R.A" }));
    meta.appendChild(el("span", { style: { opacity: ".4" }, text: "·" }));
  ...
  async function renderAnalytics() {
    ...
    // L.Y.R.A KPIs from /api/analytics/L.Y.R.A
    let L.Y.R.AData = null;
    try { L.Y.R.AData = await J.api.get("/api/analytics/L.Y.R.A?days=30"); } catch (_) {}

    if (L.Y.R.AData) {
      const kpiGrid = el("div", { class: "kpi-strip" });
      const kpis = [
        { lbl: "Requêtes 30j", val: L.Y.R.AData.total_calls||0, unit: "" },
        { lbl: "Coût 30j",     val: L.Y.R.AData.total_cost ? "$"+L.Y.R.AData.total_cost.toFixed(2) : "—", unit: "" },
        { lbl: "Tokens",       val: L.Y.R.AData.total_tokens||0, unit: "" },
      ];
      kpis.forEach(k => {
