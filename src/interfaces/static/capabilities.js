...
(function () {
  ...
  const CONFIGS = {
    "fusion360": {
      fullDesc: "Contrôle Autodesk Fusion 360 via MCP HTTP — scripts Python API, lecture et modification de designs 3D, undo/redo, export et lancement d'actions Fusion directement depuis Lyra.",
      fields: [
    ...
    "mode-streameur": {
      isRoutine: true,
      fullDesc: "Lance l'environnement stream complet — OBS Studio, scènes préconfigurées, overlay Twitch. L.Y.R.A orchestre le démarrage, gère les transitions et t'alerte si une app ne répond pas.",
  ...
  function buildViewPanel(panel, v) {
    ...
    // Statut actif
    const registered = window.top?.L.Y.R.A?.views?.list() || [];
  ...
  function renderEcosysteme() {
    ...
    const page = pageWrapper("ecosysteme", "L'écosystème L.Y.R.A", null, wrap);
    root.innerHTML = ""; root.appendChild(page);
  }
  ...
  async function buildMacropadPanel(panel) {
    ...
    body.appendChild(el("div", { class: "panel-desc",
      text: "Macropad 2 touches sur puce CH552. Configure les raccourcis, l'éclairage et flash le firmware directement depuis Lyra." }));
          ...
          try {
            ...
            if (!msgs.length) {
              ...
              msgs.forEach(m => {
                ...
                const label  = el("div", { class: "sess-bubble-role", text: m.role === "user" ? "Toi" : "L.Y.R.A" });
