...
// ── L.Y.R.ALiveKitClient ───────────────────────────────────────────────────────
class L.Y.R.ALiveKitClient {
  constructor() {
    this._room       = null;
    ...
    // Interface window.L.Y.R.A (identique à l'ancien voice.js)
    window.L.Y.R.A = {
      ...
      appendL.Y.R.AMessage: (text) => window._voiceClient?._appendAgentText(text),
  ...
  async _start() {
    ...
    const sessionId = typeof window._L.Y.R.ASessionId === "function" ? window._L.Y.R.ASessionId() : null;
    ...
    this._room.on(RoomEvent.Connected, () => {
      ...
      showVoiceStatus("L.Y.R.A en ligne");
    ...
    try {
      ...
      // On continue — la room est connectée, L.Y.R.A peut quand même parler
    }
  ...
  _appendAgentText(text) {
    ...
    if (!this._agentBubble) {
      this._agentBubble = addMsg("L.Y.R.A", "", true);
    }
...
document.addEventListener("DOMContentLoaded", () => {
  window._voiceClient = new L.Y.R.ALiveKitClient();
});
