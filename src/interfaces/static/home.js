...
(function () {
  ...
  window.addMsg = function (role, text = "", streaming = false) {
    if (streaming) {
      ...
      roleEl.className = "hcw-msg-role " + (role === "L.Y.R.A" ? "assistant" : "");
      roleEl.textContent = role === "L.Y.R.A" ? "L.Y.R.A" : "VOUS";
  ...
  let _ws = null;
  window._L.Y.R.ASessionId = () => null;
  window._L.Y.R.AWsSend = (data) => {
  ...
  function initOrb() {
    if (typeof THREE === "undefined" || typeof L.Y.R.AOrb === "undefined") {
      setTimeout(initOrb, 100);
      return;
    }
    const canvas = document.getElementById("orb-canvas");
    if (!canvas) return;
    _orb = new L.Y.R.AOrb(canvas);
    setOrbState("idle");
  }
  initOrb();
  ...
  window.addEventListener("L.Y.R.A:ws", (e) => {
    const msg = e.detail;
  ...
  async function loadChatWidget() {
    ...
    try {
      ...
      recent.forEach(m => {
        ...
        role.textContent = m.role === "assistant" ? "L.Y.R.A" : "VOUS";
  ...
  function appendChatMsg(role, text, streaming) {
    ...
    roleEl.textContent = role === "assistant" ? "L.Y.R.A" : "VOUS";
  ...
  async function sendChatMessage() {
    ...
    try {
      const sessionId = localStorage.getItem("L.Y.R.A_voice_session") || null;
      ...
      if (returnedSid) localStorage.setItem("L.Y.R.A_voice_session", returnedSid);
  ...
  document.getElementById("hcw-new-session")?.addEventListener("click", () => {
    localStorage.removeItem("L.Y.R.A_voice_session");
    const msgsEl  = document.getElementById("hcw-chat-msgs");
    const countEl = document.getElementById("hcw-chat-count");
  ...
  Lyra.navigateFrame = function (url) {
