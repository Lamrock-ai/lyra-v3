/* orb.js — L.Y.R.A Three.js sphere (vanilla JS, spec v2)
 *
 * window.createL.Y.R.AOrb(canvas, opts) → { setState, setAudioLevel, resize, destroy }
 * window.L.Y.R.AOrb(canvas)             → same (backwards compat with home.js `new L.Y.R.AOrb(…)`)
 *
...
(function () {
  ...
  window.createL.Y.R.AOrb = function (canvas, opts) {
    return createOrbInternal(canvas);
  };

  // `new L.Y.R.AOrb(canvas)` still works — constructor returning an object
  // causes `new` to return that object instead of `this`
  window.L.Y.R.AOrb = function (canvas, opts) {
    return createOrbInternal(canvas);
  };
})();
